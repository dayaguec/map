 ...
 *          Throws std::invalid_argument if id is not valid or if lane does not exist in the store.
 */
 ...
 */
inline bool isAccessOk(Lane const &lane, restriction::VehicleDescriptor const &vehicle)
{
 ...
 * @returns Width of the lane at the given longitudinal parametric offset \a longitudinalOffset
 */
...
/**
 * @return minimum duration to drive the provided parametric \a range along the \a lane based on the speed limits of the
 * lane.
 */
...
{
  return lane.direction == LaneDirection::POSITIVE || lane.direction == LaneDirection::BIDIRECTIONAL;
}
...
{
  return lane.direction == LaneDirection::NEGATIVE || lane.direction == LaneDirection::BIDIRECTIONAL;
}
...
/**
 * @brief Calculates the projection on left and right boundary of the given referencePoint.
...
/**
 * @brief Calculates the projection on left and right boundary of the parametric point on the center of the lane.
 ...
 *        Point is calculated by first calculating the projected points on left and right boundary using
 *        ProjectParametricPointToEdges(), and then parametric point t_lat between those two points.
...
{
  return getParametricPoint(lane, physics::ParametricValue(0.), physics::ParametricValue(0.5));
}
...
{
  return getParametricPoint(lane, physics::ParametricValue(1.), physics::ParametricValue(0.5));
}

/**
 * @brief Checks if Lane is longitudinally connected with another Lane at the end.
 * @param[in] other Other object. Must be IsValid()!
 * @returns True if this Lane longitudinally connected with another Lane at the end.
 */
bool isPyhsicalSuccessor(Lane const &lane, Lane const &other);

/**
 * @brief Checks if Lane is longitudinally connected with another Lane at the start.
 * @param[in] other Other object. Must be IsValid()!
 * @returns True if this Lane longitudinally connected with another Lane at the start.
 */
 ...
 *
 * @returns \c true if the ids belong to the same or neighboring lanes, false otherwise
 */
 ...
 *
 * @return true if the heading is within 90° of the lane direction, false otherwise
 */
bool isHeadingInLaneDirection(point::ParaPoint const &position, point::ENUHeading const &heading);

/**
 * @brief project a position to a neighboring lane (incl. the lane itself), which has the provided heading
 *
 * @param[in]  position to be projected
 * @param[in]  heading to be checked
 * @param[out] projectedPosition is the projected position (can be position, if the lane is already in heading
 *             direction)
 *
